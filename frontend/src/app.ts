...
function render() {
  ...
  const reconnectBtn =
    view && store.playerId && !connected && !connecting
      ? `<button id="reconnectBtn" type="button" class="ml-2 rounded-lg bg-sky-500/80 px-3 py-1.5 text-sm font-medium text-white hover:bg-sky-400">Переподключиться</button>`
      : ''
  const header = `
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-2 text-base text-slate-300">
          <button id="leaveRoom" title="Выйти из комнаты"
            class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-rose-500/90 text-white ring-1 ring-rose-500/30 hover:bg-rose-400">
...
function renderMyTeamPanel(view: WsView) {
  if (!view.me.team_id) {
    return `
      ...
      <div id="teamError" class="mt-3 hidden rounded-xl bg-rose-500/10 px-3 py-2 text-base text-rose-200 ring-1 ring-rose-500/20"></div>
    `
  }
  ...
  const players = team.players
    .map((p) => {
      ...
      return `<div class="flex items-center justify-between gap-2 rounded-xl bg-white/5 px-3 py-2 ring-1 ring-white/10">
        <div class="min-w-0">
  ...
  return `
    ...
    <div class="mt-3 grid gap-2">
      ...
      <button id="toggleRoleBtn" title="${roleDisabled ? 'После старта первого раунда роль менять нельзя' : ''}"
        class="w-full rounded-xl bg-white/10 px-3 py-2 text-base font-semibold text-white ring-1 ring-white/10 hover:bg-white/15 disabled:opacity-50"
...
function renderCenterPanel(view: WsView) {
  ...
  if (isCluegiver) {
    ...
    return `
      ...
      <div id="gameError" class="mt-3 hidden rounded-xl bg-rose-500/10 px-3 py-2 text-base text-rose-200 ring-1 ring-rose-500/20"></div>
    `
  }
...
function renderRightPanel(view: WsView) {
  ...
  const gear = `
    <button id="toggleSettings" class="rounded-xl bg-white/10 px-3 py-2 text-base font-semibold text-white ring-1 ring-white/10 hover:bg-white/15">
  ...
  const wordPackSelect =
    hasCustomPack
      ? `<div class="grid gap-1">
          ...
          <div class="rounded-xl bg-white/5 px-3 py-2 text-base text-slate-200 ring-1 ring-white/10">Свой: ${escapeHtml(view.room.custom_words_name ?? '')}</div>
        </div>`
      : `
        <label class="grid gap-1">
          <span class="text-base text-slate-300">Пак слов</span>
          <select id="cfgWordPack" class="rounded-xl bg-white/5 px-3 py-2 text-base ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500/60" ${canSettings && canChangeWordPack ? '' : 'disabled'} title="${!canChangeWordPack ? 'Пак слов нельзя менять после начала первого раунда до конца игры' : ''}">
  ...
  return `
    ...
    ${controls}
    ${history}
    <div id="rightError" class="mt-3 hidden rounded-xl bg-rose-500/10 px-3 py-2 text-base text-rose-200 ring-1 ring-rose-500/20"></div>
  `
}
                ...
                const buttons =
                  editable
                    ? `<div class="flex flex-wrap gap-1" data-round-index="${roundIdx}" data-word-index="${wordIdx}">
                        <button type="button" class="setWordOutcomeBtn rounded-lg bg-emerald-500/80 px-2 py-1 text-sm text-white hover:bg-emerald-400" data-outcome="correct">+1</button>
                        <button type="button" class="setWordOutcomeBtn rounded-lg bg-white/10 px-2 py-1 text-sm text-slate-200 ring-1 ring-white/10 hover:bg-white/20" data-outcome="dont_know">0</button>
                        <button type="button" class="setWordOutcomeBtn rounded-lg bg-rose-500/80 px-2 py-1 text-sm text-white hover:bg-rose-400" data-outcome="skip">−1</button>
                      </div>`
                    : tag
                return `<li class="flex items-center justify-between gap-3 rounded-xl bg-white/5 px-3 py-2 ring-1 ring-white/10">
