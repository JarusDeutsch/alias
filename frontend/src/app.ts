...
function render() {
  ...
  const spectatorsPanel = view
      ? (() => {
          ...
          if (gameStartedForHeader) {
            return `<div class="rounded-3xl bg-white/5 p-3 ring-1 ring-white/10 min-w-[160px]">${content}</div>`
          }
          return `<button type="button" class="becomeSpectatorBtn rounded-3xl bg-white/5 p-3 ring-1 ring-white/10 min-w-[160px] text-left hover:bg-white/10 hover:ring-white/20 focus:outline-none focus:ring-2 focus:ring-indigo-500/60 cursor-pointer transition-colors" title="Нажмите, чтобы выйти в игроки без команды">${content}</button>`
        })()
      : ''

  const reconnectBtn =
    view && store.playerId && !connected && !connecting
      ? `<button id="reconnectBtn" type="button" class="ml-2 rounded-3xl bg-sky-500/80 px-3 py-1.5 text-sm font-medium text-white hover:bg-sky-400">Переподключиться</button>`
      : ''
  const header = `
  ...
  const wordsUploadEl =
    ...
    <div class="fixed bottom-4 right-4 z-40">
      ...
      <button type="button" id="uploadWordsBtn" title="${!canChangeWordPackBtn ? 'Пак слов нельзя менять после начала первого раунда до конца игры' : view.room.custom_words_name ? 'Пак слов загружен' : 'Загрузить пак слов из CSV для этой комнаты'}"
        class="rounded-3xl bg-white/10 px-3 py-2 text-sm font-medium text-slate-200 ring-1 ring-white/10 hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-indigo-500/60 disabled:opacity-50 disabled:cursor-not-allowed"
  ...
  if (!mainEl) {
    ...
    <div id="aliasToast" class="pointer-events-none fixed bottom-4 left-1/2 z-50 hidden -translate-x-1/2 rounded-3xl bg-slate-950/80 px-3 py-1.5 text-sm text-white ring-1 ring-white/10 backdrop-blur">toast</div>
    <div id="aliasToastLeave" class="pointer-events-none fixed bottom-4 left-1/2 z-50 hidden -translate-x-1/2 rounded-3xl bg-rose-600/95 px-3 py-1.5 text-sm text-white ring-1 ring-rose-400/30 backdrop-blur">leave</div>
    `
    ...
    <div class="mt-5 grid gap-3">
      <label class="grid gap-1">
        ...
        <input id="playerName" value="${escapeHtml(store.playerName)}" placeholder="Например, Даша" maxlength="${PLAYER_NAME_MAX_LEN}"
          class="rounded-3xl bg-white/5 px-2.5 py-1.5 text-sm ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500/60" />
      </label>
      ...
      <label class="grid gap-1">
        ...
        <input id="roomCode" value="${escapeHtml(store.roomCode)}" placeholder="Например, 8K3QZP" maxlength="${ROOM_CODE_MAX_LEN}"
          class="rounded-3xl bg-white/5 px-2.5 py-1.5 text-sm uppercase tracking-widest ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500/60" />
      </label>

      ${
        fromUrl && codeDisplay
          ? `
      <div class="mt-1 rounded-3xl bg-emerald-500/15 p-4 ring-2 ring-emerald-500/40">
        <p class="text-base font-medium text-emerald-100">Вас пригласили в комнату</p>
        <button id="enterRoom" type="button" class="mt-3 w-full rounded-3xl bg-emerald-500 px-5 py-3.5 text-lg font-semibold text-emerald-950 hover:bg-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 focus:ring-offset-slate-900">
          Войти в комнату /${codeDisplay}
        </button>
      </div>
      <div class="mt-1">
        <button id="createRoom" class="min-w-[10.5rem] rounded-3xl bg-white/10 px-4 py-2 text-sm font-semibold text-slate-200 ring-1 ring-white/10 hover:bg-white/15">
          Создать свою комнату
        </button>
      </div>`
          : `
      <div class="mt-1 grid gap-2 sm:grid-cols-2">
        <button id="createRoom" class="min-w-[10.5rem] rounded-3xl bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-400">
          Создать комнату
        </button>
        <button id="enterRoom" class="min-w-[10.5rem] rounded-3xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-emerald-950 hover:bg-emerald-400 disabled:opacity-50" ${store.roomCode.trim() ? '' : 'disabled'}>
          Войти по коду
        </button>
      </div>`
      }
      ...
      <div id="lobbyRoomCodeRow">${
        store.roomCode
          ? `<div class="flex flex-wrap items-center gap-2 text-base text-slate-400">
              ...
              <button id="loadRoom" type="button" class="rounded-3xl bg-white/10 px-2 py-1 text-sm text-slate-300 hover:bg-white/15">Обновить</button>
            </div>`
          : `<div class="text-base text-slate-400"></div>`
      }</div>

      ${store.lobbyLoadingMessage ? `<div id="lobbyStatus" class="rounded-3xl bg-white/5 px-3 py-2 text-base text-slate-300 ring-1 ring-white/10 flex items-center gap-2"><span class="alias-spinner"></span><span>${escapeHtml(store.lobbyLoadingMessage)}</span></div>` : '<div id="lobbyStatus" class="hidden rounded-3xl bg-white/5 px-3 py-2 text-base text-slate-300 ring-1 ring-white/10"></div>'}
      <div id="lobbyError" class="hidden rounded-3xl bg-rose-500/10 px-3 py-2 text-base text-rose-200 ring-1 ring-rose-500/20 mt-2"></div>
    </div>
  `
}
...
function renderHelpCard() {
  return `
    ...
    <ul class="mt-4 grid gap-2 text-base text-slate-300">
      <li class="rounded-3xl bg-white/5 px-3 py-2 ring-1 ring-white/10">
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-[1fr_minmax(0,960px)_1fr]">
      ...
      <div class="alias-mobile-tabs min-w-0">
        <div class="flex gap-2 border-b border-white/10 pb-2">
          <button type="button" class="alias-mobile-tab rounded-3xl px-4 py-2.5 text-sm font-semibold transition-colors ${mobileTab === 'team' ? 'bg-white/15 text-white ring-1 ring-white/20' : 'bg-white/5 text-slate-400 ring-1 ring-white/10 hover:bg-white/10'}" data-tab="team">Команда</button>
          <button type="button" class="alias-mobile-tab rounded-3xl px-4 py-2.5 text-sm font-semibold transition-colors ${mobileTab === 'controls' ? 'bg-white/15 text-white ring-1 ring-white/20' : 'bg-white/5 text-slate-400 ring-1 ring-white/10 hover:bg-white/10'}" data-tab="controls">Управление</button>
        </div>
...
function renderTopBar(view: WsView) {
  ...
  const teams = teamsList
    .map((t) => {
      ...
      if (gameStarted) {
        return `<div class="flex items-start gap-2 rounded-3xl bg-white/5 px-2.5 py-1.5 ring-1 ring-white/10">${base}</div>`
      }
      return `<div role="button" tabindex="0" data-teamcode="${escapeHtml(t.code)}" class="joinTeamBtn flex w-full cursor-pointer items-start gap-2 rounded-3xl bg-white/5 px-2.5 py-1.5 text-left ring-1 ring-white/10 transition-colors hover:bg-white/10 hover:ring-white/20 focus:outline-none focus:ring-2 focus:ring-indigo-500/60" title="Нажмите, чтобы вступить в команду">${base}</div>`
    })
    .join('')
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-2 text-base text-slate-300">
          <button id="leaveRoom" title="Выйти из комнаты"
            class="inline-flex h-8 w-8 items-center justify-center rounded-3xl bg-rose-500/90 text-white ring-1 ring-rose-500/30 hover:bg-rose-400">
...
function renderTeamChooser(view: WsView) {
  ...
  const teamBtnClass = gameStarted
    ? 'joinTeamBtn flex items-center justify-between gap-2 rounded-3xl bg-white/5 px-3 py-2 text-base ring-1 ring-white/10 cursor-not-allowed opacity-60'
    : 'joinTeamBtn flex items-center justify-between gap-2 rounded-3xl bg-white/5 px-3 py-2 text-base ring-1 ring-white/10 hover:bg-white/10 cursor-pointer'
  ...
  const plus = `
    <button id="addTeamBtn" title="Создать команду (+) и вступить"
      class="inline-flex h-10 w-10 items-center justify-center rounded-3xl bg-emerald-500 text-xl font-semibold text-emerald-950 hover:bg-emerald-400 disabled:opacity-50"
...
function renderMyTeamPanel(view: WsView) {
  if (!view.me.team_id) {
    return `
      ...
      <div id="teamError" class="mt-3 hidden rounded-3xl bg-rose-500/10 px-3 py-2 text-base text-rose-200 ring-1 ring-rose-500/20"></div>
    `
  }
  ...
  const players = team.players
    .map((p) => {
      ...
      return `<div class="flex items-center justify-between gap-2 rounded-3xl bg-white/5 px-3 py-2 ring-1 ring-white/10">
        <div class="min-w-0">
  ...
  return `
    ...
    <div class="mt-3 grid gap-2">
      ...
      <button id="toggleRoleBtn" title="${roleDisabled ? 'После старта первого раунда роль менять нельзя' : ''}"
        class="w-full rounded-3xl bg-white/10 px-3 py-2 text-base font-semibold text-white ring-1 ring-white/10 hover:bg-white/15 disabled:opacity-50"
...
function renderCenterPanel(view: WsView) {
  if (view.room.game_over) {
    ...
    const teamsList = teams
      .map((t) => {
        ...
        return `<div class="rounded-3xl px-4 py-3 ring-1 ${isWinner ? 'bg-emerald-500/15 ring-emerald-500/30' : 'bg-white/5 ring-white/10'}">
    ...
    return `
      ...
      <div class="alias-game-over-confetti relative mt-3 overflow-hidden rounded-3xl bg-emerald-500/10 py-6 px-5 ring-2 ring-emerald-500/30">
      ...
      ${
        canRestart
          ? `<button id="restartGame" class="mt-5 w-full rounded-3xl bg-white/10 px-4 py-3 text-base font-semibold text-white ring-1 ring-white/10 hover:bg-white/15 transition-opacity">
              Перезапустить игру
            </button>`
          : ''
      }
  ...
  if (isCluegiver) {
    ...
    return `
      ...
      <div class="mt-6 rounded-3xl bg-gradient-to-b from-white/12 to-white/5 p-7 ring-1 ring-white/10">
      ...
      <div class="mt-5 alias-outcome-buttons rounded-3xl p-3 ring-1 ring-white/10 ${timeExpired ? 'alias-outcome-buttons-expired' : ''}">
        <div class="grid gap-3 sm:grid-cols-3">
          <button id="markCorrect" class="rounded-3xl bg-emerald-500 px-4 py-3 text-base font-semibold text-emerald-950 hover:bg-emerald-400 disabled:opacity-50" ${
          team.round_active ? '' : 'disabled'
        } title="Клавиша 1">
            Угадал (+1)
          </button>
          <button id="markDontKnow" class="rounded-3xl bg-white/10 px-4 py-3 text-base font-semibold text-white ring-1 ring-white/10 hover:bg-white/15 disabled:opacity-50" ${
          team.round_active ? '' : 'disabled'
        } title="Клавиша 2">
            Не знаю (0)
          </button>
          <button id="markSkip" class="rounded-3xl bg-rose-500/90 px-4 py-3 text-base font-semibold text-white hover:bg-rose-400 disabled:opacity-50" ${
          team.round_active ? '' : 'disabled'
        } title="Клавиша 3">
            Пропуск (-1)
          </button>
      ...
      <div class="mt-4 rounded-3xl bg-white/5 px-3 py-2 text-base text-slate-300 ring-1 ring-white/10">
        Последнее показанное угадывающим: <span class="font-semibold text-slate-100">${escapeHtml(team.last_revealed_word ?? '—')}</span>
      </div>

      <div id="gameError" class="mt-3 hidden rounded-3xl bg-rose-500/10 px-3 py-2 text-base text-rose-200 ring-1 ring-rose-500/20"></div>
    `
  }
...
function renderRightPanel(view: WsView) {
  ...
  const dice = `
    <button id="randomizeBtn" title="Рандомно распределить игроков по командам и ролям"
      class="inline-flex h-9 w-9 items-center justify-center rounded-3xl bg-white/10 text-white ring-1 ring-white/10 hover:bg-white/15 disabled:opacity-40"
  ...
  const gear = `
    <button id="toggleSettings" class="rounded-3xl bg-white/10 px-3 py-2 text-base font-semibold text-white ring-1 ring-white/10 hover:bg-white/15">
  ...
  const wordPackSelect =
    hasCustomPack
      ? `<div class="grid gap-1">
          ...
          <div class="rounded-3xl bg-white/5 px-3 py-2 text-base text-slate-200 ring-1 ring-white/10">Свой: ${escapeHtml(view.room.custom_words_name ?? '')}</div>
        </div>`
      : `
        <label class="grid gap-1">
          <span class="text-base text-slate-300">Пак слов</span>
          <select id="cfgWordPack" class="rounded-3xl bg-white/5 px-3 py-2 text-base ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500/60" ${canSettings && canChangeWordPack ? '' : 'disabled'} title="${!canChangeWordPack ? 'Пак слов нельзя менять после начала первого раунда до конца игры' : ''}">
  ...
  const restartInSettingsBtn =
    canRestartInGame
      ? `<button id="restartGameInSettings" type="button" class="mt-3 w-full rounded-3xl bg-amber-500/20 px-4 py-3 text-base font-semibold text-amber-200 ring-1 ring-amber-500/30 hover:bg-amber-500/30">
          Перезапустить игру
        </button>`
      : ''
    ...
    store.settingsOpen
      ...
      <div class="mt-4 grid gap-3">
        ...
        <label class="grid gap-1">
          ...
          <select id="cfgMode" class="rounded-3xl bg-white/5 px-3 py-2 text-base ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500/60" ${
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="grid gap-1">
            ...
            <input id="cfgRoundSec" type="number" min="${CONFIG_ROUND_SEC_MIN}" max="${CONFIG_ROUND_SEC_MAX}" value="${cfg.round_seconds}"
              class="rounded-3xl bg-white/5 px-3 py-2 text-base ring-1 ring-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500/60" ${canSettings ? '' : 'disabled'} />
          </label>
          <label class="grid gap-1">
  ...
  const controls =
    view.me.role === 'cluegiver' && view.me.team_id
      ...
      <div class="mt-4 grid gap-3 sm:grid-cols-2">
        <button id="startRound" class="rounded-3xl bg-indigo-500 px-4 py-3 text-base font-semibold text-white hover:bg-indigo-400 disabled:opacity-50" ${
          team && !team.round_active && !view.room.game_over ? '' : 'disabled'
        }>
          Старт раунда
        </button>
        <button id="endRound" class="rounded-3xl bg-rose-500/90 px-4 py-3 text-base font-semibold text-white hover:bg-rose-400 disabled:opacity-50" ${
  ...
  const history = team
    ? `
      <div class="mt-6 rounded-3xl bg-white/5 p-5 ring-1 ring-white/10">
        <div class="text-base text-slate-300">История слов (текущий раунд)</div>
        <div class="mt-3 max-h-[510px] overflow-y-auto pr-1">
          ${renderRoundHistory(team, true, !team.round_active)}
        </div>
  ...
  return `
    ...
    ${controls}
    ${history}
    <div id="rightError" class="mt-3 hidden rounded-3xl bg-rose-500/10 px-3 py-2 text-base text-rose-200 ring-1 ring-rose-500/20"></div>
  `
}
                ...
                const buttons =
                  editable
                    ? `<div class="flex flex-wrap gap-1" data-round-index="${roundIdx}" data-word-index="${wordIdx}">
                        <button type="button" class="setWordOutcomeBtn rounded-3xl bg-emerald-500/80 px-2 py-1 text-sm text-white hover:bg-emerald-400" data-outcome="correct">+1</button>
                        <button type="button" class="setWordOutcomeBtn rounded-3xl bg-white/10 px-2 py-1 text-sm text-slate-200 ring-1 ring-white/10 hover:bg-white/20" data-outcome="dont_know">0</button>
                        <button type="button" class="setWordOutcomeBtn rounded-3xl bg-rose-500/80 px-2 py-1 text-sm text-white hover:bg-rose-400" data-outcome="skip">−1</button>
                      </div>`
                    : tag
                return `<li class="flex items-center justify-between gap-3 rounded-3xl bg-white/5 px-3 py-2 ring-1 ring-white/10">
