...
## Почему не срабатывает авто-деплой при коммите

Cloudflare Pages запускает сборку **при пуше в GitHub**, а не при локальном коммите. Проверьте по шагам:

1. **Вы пушите в удалённый репозиторий?**  
   После `git commit` обязательно выполните `git push origin main` (или вашу ветку). Пока изменения только локально — деплоя не будет.

2. **Ветка совпадает с Production branch?**  
   В **Cloudflare Dashboard** → ваш проект Pages → **Settings** → **Builds & deployments** → **Production branch** должна быть указана ветка, в которую вы пушите (чаще всего `main`). Пуш в другую ветку создаёт только Preview-деплой.

3. **Подключение GitHub не отозвано?**  
   **Settings** → **Builds & deployments** → блок **Build configuration** / **Connected repository**. Если написано, что репозиторий отключён или нужно заново авторизовать — нажмите **Reconnect** и заново выберите репозиторий и права (доступ к репо).

4. **Сборки не отключены?**  
   В тех же настройках проверьте, что нет опции вроде «Skip builds» или «Disable automatic deployments» для production.

5. **Проверьте вкладку Deployments:**  
   После `git push` откройте **Deployments**. Появился ли новый деплой (хотя бы в статусе Building)? Если да, но он падает с ошибкой — смотрите лог сборки (клик по деплою). Если нового деплоя нет вообще — проблема в пунктах 1–4 (чаще всего забыли сделать push или подключение Git слетело).
